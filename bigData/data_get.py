...
def close_windows():
    ...
    try:
        time.sleep(5)
        if dr.find_element_by_class_name("sign-form").find_element_by_class_name("icon-close"):
            dr.find_element_by_class_name("sign-form").find_element_by_class_name("icon-close").click()
...
def get_current_region_job(k_index):
    ...
    while (flag == 0):
        ...
        for job in job_list:#获取当前页的职位30条
            ...
            split_str = re.search(r'[li-zA-Z =<>/"]{23}', experience_education_raw)  # 搜索分割字符串<em class="vline"></em>
            # print(split_str)

            experience_education_replace = re.sub(r'[li-zA-Z =<>/"]{23}', ",", experience_education_raw)  # 分割字符串替换为逗号
            ...
            if len(experience_education_list)!=5:
...
def main():
    ...
    for i in range(3,17,1):
        ...
        # 获取城市
        close_windows()
        hot_city_list = dr.find_element_by_class_name("dropdown-city-list").find_elements_by_tag_name("li")
        print(hot_city_list)
        close_windows()
        ...
        # close_windows()
        # hot_city_list = dr.find_element_by_class_name("dropdown-city-list").find_elements_by_tag_name("li")
        # print('城市：{}'.format(i-5),hot_city_list[i].text)
        # time.sleep(0.5)
        ...
        for j in range(1,50,1):
            ...
            #输出区县名称
            close_windows()
            city_district = dr.find_element_by_class_name("dropdown-area-list").find_elements_by_tag_name("li")
